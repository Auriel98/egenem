import { motion } from 'framer-motion'

const navLinks = [
  { name: 'Accueil', id: 'hero' },
  { name: 'À propos', id: 'about' },
  { name: 'Prestations', id: 'services' },
  { name: 'Pourquoi nous', id: 'whyus' },
  { name: 'Contact', id: 'contact' },
]

// Icons
const PhoneIcon = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M22 16.92v3a2 2 0 0 1-2.18 2A19.79 19.79 0 0 1 3.1 4.18 2 2 0 0 1 5.1 2h3a2 2 0 0 1 2 1.72c.13.96.37 1.9.7 2.81a2 2 0 0 1-.45 2.11L9.91 9.91a16 16 0 0 0 6 6l.91-.91a2 2 0 0 1 2.11-.45c.91.33 1.85.57 2.81.7A2 2 0 0 1 22 16.92z"/>
  </svg>
)

const MailIcon = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <rect x="2" y="4" width="20" height="16" rx="2"/>
    <path d="m22 7-10 6L2 7"/>
  </svg>
)

const MapPinIcon = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M20 10c0 6-8 12-8 12S4 16 4 10a8 8 0 0 1 16 0Z"/>
    <circle cx="12" cy="10" r="3"/>
  </svg>
)

// Data
const contacts = [
  { Icon: PhoneIcon, label: 'Téléphone', value: '+241 76 48 51 46' },
  { Icon: MailIcon, label: 'Email', value: 'contact@egenem.ga' },
  { Icon: MapPinIcon, label: 'Adresse', value: 'Port-Gentil, Gabon' },
]

const chips = [
  'Nettoyage Industriel',
  'Haute Pression',
  'Espaces Verts',
  'Structures Métalliques',
]

export default function Footer() {
  return (
    <footer style={{ background: '#0a1f14', color: '#fff' }}>

      {/* TOP */}
      <div style={{
        padding: '60px 40px',
        display: 'grid',
        gridTemplateColumns: '1.5fr 1fr 1fr',
        gap: 40,
      }}>

        {/* BRAND */}
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
          <h2 style={{ color: '#c9a227' }}>EGENEM</h2>
          <p style={{ opacity: 0.6 }}>
            Entreprise de nettoyage moderne au Gabon.
          </p>
        </motion.div>

        {/* NAVIGATION */}
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
          <h4 style={{ color: '#c9a227' }}>Navigation</h4>

          {navLinks.map(link => (
            <a
              key={link.id}
              href={`#${link.id}`}
              style={{
                display: 'block',
                marginTop: 10,
                color: '#ccc',
                textDecoration: 'none'
              }}
            >
              {link.name}
            </a>
          ))}
        </motion.div>

        {/* CONTACT */}
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
          <h4 style={{ color: '#c9a227' }}>Contact</h4>

          {contacts.map(c => (
            <div key={c.label} style={{ display: 'flex', gap: 10, marginTop: 10 }}>
              <c.Icon />
              <span>{c.value}</span>
            </div>
          ))}
        </motion.div>
      </div>

      {/* CHIPS */}
      <div style={{
        borderTop: '1px solid rgba(255,255,255,.1)',
        padding: 20,
        display: 'flex',
        flexWrap: 'wrap',
        gap: 10
      }}>
        {chips.map(c => (
          <span key={c} style={{
            border: '1px solid #1a4a35',
            padding: '5px 10px',
            borderRadius: 20,
            fontSize: 12
          }}>
            {c}
          </span>
        ))}
      </div>

      {/* BOTTOM */}
      <div style={{
        borderTop: '1px solid rgba(255,255,255,.1)',
        padding: 20,
        textAlign: 'center',
        fontSize: 12,
        opacity: 0.6
      }}>
        © {new Date().getFullYear()} EGENEM — Tous droits réservés
      </div>

    </footer>
  )
}